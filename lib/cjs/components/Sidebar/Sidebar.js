...
export const Sidebar = ({ children, className = '', breakPoint = 'md', ...rest }) => {
    ...
    return (_jsx("aside", { className: cn('bg-bg fixed top-0 left-0 z-40 h-screen w-64 flex-col p-5 transition-all duration-300 ease-in-out', breakpointStyle(breakPoint), isOpen ? 'translate-x-0' : '-translate-x-full', className), ...rest, children: children }));
};
